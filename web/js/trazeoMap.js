...
// Routing Machine plugin info
    ...
    var routing = new L.Routing({
        ...
        ,icons: {
            ...
            ,normal: new L.Icon({iconUrl:mapConfig.iconImagePath+'/marker-icon.png', iconSize: [22,35],  iconAnchor: [10, 35]})
        }
        ,snapping: {
            layers: []
        }
...
// map custom events
    ...
    L.Map.prototype.loadFromEvents=function(events) {
        ...
        if(typeof poly==='undefined') {
            poly=new L.polyline(mapConfig.events[i].latLng,{dashArray: "10, 20",color:"green"});
            poly.addTo(map);
            i++
        }

        for(;i<mapConfig.events.length;i++){
    ...
    L.Map.prototype.loadFromWaypoints=function(waypoints) {
        for(var index=0; waypoints.length>index;index++) {
            ...
            if(marker.pickUp) {
                map.createPickUpWaypoint(marker,waypoints[index].pickUpText);
            }
        }
    };
    ...
    $("form").submit(function(){
        ...
        var distance=kml?map.getDistance(arrayPoints):map.getDistance();
...
// methods
    ...
    L.Map.prototype.showResumeInfo=function(){
        if(mapConfig.events){
            ...
            if(mapConfig.events.length!=1){
                var last=mapConfig.events[mapConfig.events.length-2].latLng;
            }
            else last=first;
            var distance=0;
            if(first && mapConfig.resumeValues.startSite==null){
